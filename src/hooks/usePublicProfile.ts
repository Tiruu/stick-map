...
import { getFriendshipBetween, sendFriendRequest } from "../services/friends";

type UserLike = {
  id: string;
} | null;

export function usePublicProfile(user: UserLike) {
  const [publicProfile, setPublicProfile] = useState<Profile | null>(null);

  const [publicProfileFriendship, setPublicProfileFriendship] =
  ...
  const openPublicProfile = useCallback(
    async (profileId: string) => {
      ...
      try {
        ...
        const friendship = await getFriendshipBetween(user.id, profileId);
  ...
  const handleSendFriendRequest = useCallback(async () => {
    ...
    try {
      const friendship = await sendFriendRequest(user.id, publicProfile.id);
